view: x_eop_claimcounts_triangle_treaty_accident_month {
  derived_table: {
    sql: Select
        YEAR(ProcessingDate)*100+MONTH(ProcessingDate) as transmonth,
        CAST('M' + RIGHT('00' + CAST(DATEDIFF(m, t.eff_date, loss_date) + 1 as varchar(3)), 3) as varchar(4)) as lagmonth,
        YEAR(loss_date)*100+MONTH(loss_date) As accidentmonth,
        company_id,
        state_id,
        z.lob_id,
  ...
  WHERE z.loss_date > z.eff_date
    GROUP BY
      YEAR(ProcessingDate)*100+MONTH(ProcessingDate),
        CAST('M' + RIGHT('00' + CAST(DATEDIFF(m, t.eff_date, Loss_date) + 1 as varchar(3)), 3) as varchar(4)),

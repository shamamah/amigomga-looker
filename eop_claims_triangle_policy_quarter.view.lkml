view: eop_claims_triangle_policy_quarter {
  derived_table: {
    sql:SELECT
            CASE WHEN DATEDIFF(q, '2019-05-01', PolicyImage.eff_date) < 0 THEN 0 ELSE DATEDIFF(q, '2019-05-01', PolicyImage.eff_date) END as policy_quarter,
            CASE WHEN DATEDIFF(q, PolicyImage.eff_date, CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01') < 0 THEN 0 ELSE DATEDIFF(q, PolicyImage.eff_date, CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01') END
            --CASE WHEN DATEDIFF(m, '2019-05-01', z.eff_date) < 0 THEN 0 ELSE DATEDIFF(m, '2019-05-01', z.eff_date) END
                    as lag_quarter,
            DATEDIFF(q, '2019-05-01', CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01') as trans_quarter,
  ...
  --and loss_date < policyimage.eff_date
    GROUP BY
    CASE WHEN DATEDIFF(q, '2019-05-01', PolicyImage.eff_date) < 0 THEN 0 ELSE DATEDIFF(q, '2019-05-01', PolicyImage.eff_date) END,
    CASE WHEN DATEDIFF(q, PolicyImage.eff_date, CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01') < 0 THEN 0 ELSE DATEDIFF(q, PolicyImage.eff_date, CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01') END
  --CASE WHEN DATEDIFF(m, '2019-05-01', z.eff_date) < 0 THEN 0 ELSE DATEDIFF(m, '2019-05-01', z.eff_date) END
    ,
    DATEDIFF(q, '2019-05-01', CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01'),
...
;;
    ...
    dimension: itd_claims_primarykey {
      primary_key: yes
      hidden: yes
      sql: CONCAT(${TABLE}.lob_id, ' ', ${TABLE}.coveragecode_id, ' ', ${TABLE}.trans_quarter, ' ', ${TABLE}.lag_quarter, ' ', ${TABLE}.NewRen, ' ', ${TABLE}.Treaty, ' ', ${TABLE}.policy_quarter);;

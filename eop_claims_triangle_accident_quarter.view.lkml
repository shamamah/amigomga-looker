view: eop_claims_triangle_accident_quarter {
  derived_table: {
    sql: SELECT
        CONCAT(CFE.year, CASE WHEN CFE.month in (1,2,3) THEN '1'
                                              WHEN CFE.month in (4,5,6) THEN '2'
                                              WHEN CFE.month in (7,8,9) THEN '3'
                                              WHEN CFE.month in (10,11,12) THEN '4'
                                              END) as w_quarter,
        CAST('Q' + CAST(DATEDIFF(q, ClaimControl.loss_date, CAST(cfe.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(cfe.month as varchar(2)), 2) as varchar(2)) + '-01') + 1 as varchar(1)) as varchar(2)) as QuarterID,
        CONCAT(YEAR(ClaimControl.loss_date), CASE WHEN MONTH(ClaimControl.loss_date) in (1,2,3) THEN '1'
                                              WHEN MONTH(ClaimControl.loss_date) in (4,5,6) THEN '2'
                                              WHEN MONTH(ClaimControl.loss_date) in (7,8,9) THEN '3'
                                              WHEN MONTH(ClaimControl.loss_date) in (10,11,12) THEN '4'
            ...
            INNER JOIN Customer_Reports.dbo.Treaty t
              ...
              GROUP BY
                CONCAT(CFE.year, CASE WHEN CFE.month in (1,2,3) THEN '1'
                                              ...
                                              WHEN CFE.month in (10,11,12) THEN '4'
                                              END),
                CAST('Q' + CAST(DATEDIFF(q, ClaimControl.loss_date, CAST(cfe.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(cfe.month as varchar(2)), 2) as varchar(2)) + '-01') + 1 as varchar(1)) as varchar(2)),
                CONCAT(YEAR(ClaimControl.loss_date), CASE WHEN MONTH(ClaimControl.loss_date) in (1,2,3) THEN '1'
                                              WHEN MONTH(ClaimControl.loss_date) in (4,5,6) THEN '2'
                                              WHEN MONTH(ClaimControl.loss_date) in (7,8,9) THEN '3'
                                              WHEN MONTH(ClaimControl.loss_date) in (10,11,12) THEN '4'

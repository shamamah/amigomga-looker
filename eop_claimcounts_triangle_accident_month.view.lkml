  ...
  derived_table: {
      sql: Select
        YEAR(ProcessingDate)*100+MONTH(ProcessingDate) as transmonth,
          CAST('M' + RIGHT('00' + CAST(DATEDIFF(m, loss_date, ProcessingDate) + 1 as varchar(3)), 3) as varchar(4)) as lagmonth,
      ...
      GROUP BY
        YEAR(ProcessingDate)*100+MONTH(ProcessingDate),
          CAST('M' + RIGHT('00' + CAST(DATEDIFF(m, loss_date, ProcessingDate) + 1 as varchar(3)), 3) as varchar(4)),

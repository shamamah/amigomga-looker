view: eop_claims_triangle_policy_month {
  derived_table: {
    sql:SELECT
              CASE WHEN DATEDIFF(m, '2019-05-01', PolicyImage.eff_date) < 0 THEN 0 ELSE DATEDIFF(m, '2019-05-01', PolicyImage.eff_date) END as policy_month,
              CASE WHEN DATEDIFF(m, PolicyImage.eff_date, CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01') < 0 THEN 0 ELSE DATEDIFF(m, PolicyImage.eff_date, CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01') END
    ...
    --and loss_date < policyimage.eff_date
      GROUP BY
      CASE WHEN DATEDIFF(m, '2019-05-01', PolicyImage.eff_date) < 0 THEN 0 ELSE DATEDIFF(m, '2019-05-01', PolicyImage.eff_date) END,
      CASE WHEN DATEDIFF(m, PolicyImage.eff_date, CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01') < 0 THEN 0 ELSE DATEDIFF(m, PolicyImage.eff_date, CAST(CFE.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(CFE.month as varchar(2)), 2) as varchar(2)) + '-01') END
    --CASE WHEN DATEDIFF(m, '2019-05-01', z.eff_date) < 0 THEN 0 ELSE DATEDIFF(m, '2019-05-01', z.eff_date) END
      ,
  ...
  ;;
    ...
    dimension: itd_claims_primarykey {
      primary_key: yes
      hidden: yes
      sql: CONCAT(${TABLE}.lob_id, ' ', ${TABLE}.coveragecode_id, ' ', ${TABLE}.trans_month, ' ', ${TABLE}.lag_month, ' ', ${TABLE}.NewRen, ' ', ${TABLE}.Treaty, ' ', ${TABLE}.policy_month, ' ', ${TABLE}.effmonth);;

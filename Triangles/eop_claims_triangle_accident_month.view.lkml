view: eop_claims_triangle_accident_month {
  derived_table: {
    sql: SELECT
        CFE.year*100+CFE.month as transmonth,
        CAST('M' + RIGHT('00' + CAST(DATEDIFF(m, ClaimControl.loss_date, CAST(cfe.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(cfe.month as varchar(2)), 2) as varchar(2)) + '-01') + 1 as varchar(3)), 3) as varchar(4)) as lagmonth,
        YEAR(ClaimControl.loss_date)*100+MONTH(ClaimControl.loss_date) As accidentmonth,
        V.company_id,
        V.state_id,
        V.lob_id,
...
--          Treaty_Name + ' (' + CAST(Treaty_num as varchar(2)) + ')' as Treaty,
        ...
        INNER JOIN Customer_Reports.dbo.Treaty t
                ...
                AND t.companystatelob_id = v.companystatelob_id
                AND PolicyImage.eff_date between t.eff_date and t.exp_date
    WHERE
      CFE.claimeoplevel_id = 3
    GROUP BY
      CFE.year*100+CFE.month,
      CAST('M' + RIGHT('00' + CAST(DATEDIFF(m, ClaimControl.loss_date, CAST(cfe.year as varchar(4)) + '-' + CAST(RIGHT('00' + CAST(cfe.month as varchar(2)), 2) as varchar(2)) + '-01') + 1 as varchar(3)), 3) as varchar(4)),
      YEAR(ClaimControl.loss_date)*100+MONTH(ClaimControl.loss_date),
      V.company_id,
      V.state_id,
      V.lob_id,
  ...
  dimension: itd_claims_primarykey {
    ...
    #                   ${TABLE}.w_quarter)  ;;
    sql: CONCAT(${TABLE}.lob_id, ' ', ${TABLE}.coveragecode_id, ' ', ${TABLE}.transmonth, ' ', ${TABLE}.lagmonth, ' ', ${TABLE}.accidentmonth, ' ', ${TABLE}.NewRen);;
  }

...
left JOIN (Select a.policy_id, a.policyimage_num, t.zip, t.city, t.version_id, t.territory_num territory_num
        from dbo.Address A
      Join (Select policy_id, policyimage_num, max(address_id) maxadid from dbo.Address
       ...
       left JOIN (select zip, city, version_id, MAX(Territory_num) Territory_num from [ProductionBackup].[dbo].[Territory]
...
left join (Select distinct x.city, x.version_id, territory_num from [ProductionBackup].[dbo].[Territory] x
